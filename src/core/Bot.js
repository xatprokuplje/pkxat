...
export class Bot {
    ...
    async init () {
        try {
            await fs.mkdir(`${process.env.DATA_DIR || "."}/cache`, { recursive: true });
            await fs.mkdir(`${process.env.DATA_DIR || "."}/logs`, { recursive: true });
    ...
    async stop () {
        ...
        if (this.state.messageRotationInterval) {
            clearInterval(this.state.messageRotationInterval);
            this.state.messageRotationInterval = null;
        }
     ...
     * configured messages to the chat room one by one, in a loop, waiting
     * `messageIntervalMs` between each send. Safe to call again any time
     * (e.g. after the message list or interval changes) - it just restarts
     * the loop with the current values.
     */
    startMessageRotation () {
        ...
        this.state.messageRotationInterval = runIfConnected(async () => {
            const messages = this.state.autoMessages;
            if (!messages.length) return;

            const message = messages[this.state.autoMessageIndex % messages.length];
            this.state.autoMessageIndex++;
            await this.sendMessage(message);
        }, this, this.state.messageIntervalMs);
    }

    /**
     * Updates the auto-message list and/or interval for this account and
     * restarts the rotation with the new values.
     * @param {string[]} messages - Messages to send, in order, in a loop.
     * @param {number} [intervalMs] - Delay between messages, in ms.
     */
    async setMessages (messages, intervalMs) {
        this.state.autoMessages = Array.isArray(messages) ? messages : [];
        if (intervalMs) this.state.messageIntervalMs = intervalMs;
        this.state.autoMessageIndex = 0;
        this.startMessageRotation();
    }

    /**
     * Marks/unmarks this account as the "primary" one that sends the
     * welcome message on user join (see UserJoinedHandler). No restart
     * needed - just flips a flag checked on the next "u" packet.
     * @param {boolean} isPrimary
     */
    setPrimary (isPrimary) {
        this.state.isPrimary = Boolean(isPrimary);
    }
